...
void rv32_keccakp_split(uint32_t v[50])
{
	...
	for (p = v; p != &v[50]; p += 2) {
		...
		t0 = rvb_unshfl(p[0], 15);
		t1 = rvb_unshfl(p[1], 15);
...
void rv32_keccakp(void *s)
{
	...
	for (q = rc; q != &rc[48]; q += 2) {
		...
		for (p = v; p <= &v[40]; p += 10) {	//  (5 iterations)
			...
			t0 = rvb_andn(t8, t6);
			t1 = rvb_andn(t9, t7);
			t8 = t8 ^ rvb_andn(t2, u0);
			t9 = t9 ^ rvb_andn(t3, u1);
			t2 = t2 ^ rvb_andn(t6, t4);
			t3 = t3 ^ rvb_andn(t7, t5);
			t6 = t6 ^ rvb_andn(u0, t8);
			t7 = t7 ^ rvb_andn(u1, t9);
			u0 = u0 ^ rvb_andn(t4, t2);
			u1 = u1 ^ rvb_andn(t5, t3);
			t4 = t4 ^ t0;
			t5 = t5 ^ t1;
			p[0] = u0;
			p[2] = t2;
